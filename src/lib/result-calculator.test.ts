...
test("calculate results for simple score with a single part being played", () => {
  ...
  const score: Score = [
    [{ notes: ["SNARE"] }, { notes: ["SNARE"] }, { notes: ["SNARE"] }, { notes: ["SNARE"] }],
    [{ notes: ["SNARE"] }, { notes: ["SNARE"] }, { notes: ["SNARE"] }, { notes: ["SNARE"] }],
  ];

  assert.deepStrictEqual(calculateResult({ ticks, notesPlayed, score, graceTime: 0 }), {
    missed: 0,
    right: 8,
  });
});
...
test("group notes for a tick inside the grace time", () => {
  ...
  assert.deepStrictEqual(calculateResult({ ticks, notesPlayed, score, graceTime: 2 }), {
    missed: 0,
    right: 8,
  });
});
...
test("real world example", { only: true }, () => {
  const props: CalculateResultProps = {
    ticks: [
      31710.5, 32460, 33210.200000047684, 33959.80000001192, 34710.60000002384, 35460.200000047684,
      36210.80000001192, 36960.700000047684, 37709.5, 38460, 39209.60000002384, 39959.90000003576,
      40710.200000047684, 41459.5, 42209.90000003576, 42959.700000047684, 43710.200000047684,
      44459.90000003576,
    ],
    notesPlayed: [
      {
