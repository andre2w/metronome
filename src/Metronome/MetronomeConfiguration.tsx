...
export function MetronomeConfiguration({ configuration, started, onChange, toggle }: MetronomeConfigurationProps) {
  ...
  return <Flex gap="5">
    <Flex direction="column">
      <Text>Notes</Text>
      <Select.Root value={String(configuration.notes)} onValueChange={value => onChange({ ...configuration, notes: Number(value) })} defaultValue="4">
        <Select.Trigger />
        <Select.Content>
    ...
    <Flex direction="column">
      <Text>BPM</Text>
      <TextField.Root type="number" value={configuration.beats} onChange={e => onChange({ ...configuration, beats: Number(e.target.value) })} step={1} />
    </Flex>
    <Flex direction="column">
